...
resource "azurerm_subnet" "proxy" {
  ...
  resource_group_name  = azurerm_resource_group.main.name
  virtual_network_name = azurerm_virtual_network.main.name
  address_prefixes     = [var.subnet_proxy_prefix]
}
...
# - Allow HTTP/3000 from anywhere (public access)
# -----------------------------------------------------------------------------
...
resource "azurerm_network_security_group" "proxy" {
  ...
  security_rule {
    ...
    protocol                   = "Tcp"
    source_port_range          = "*"
    destination_port_range     = "6033"
    source_address_prefix      = var.subnet_app_prefix
    destination_address_prefix = "*"
  }

  security_rule {
    name                       = "Allow-Admin-From-App"
    priority                   = 120
    direction                  = "Inbound"
    access                     = "Allow"
    protocol                   = "Tcp"
...
# NSG: Database
# - Allow SSH from anywhere (for management)
# - Allow MySQL (3306) from ProxySQL subnet only
# - Allow MySQL (3306) within DB subnet (for replication)
# -----------------------------------------------------------------------------
resource "azurerm_network_security_group" "db" {
  ...
  security_rule {
    ...
    source_address_prefix      = var.subnet_proxy_prefix
    destination_address_prefix = "*"
  }
  ...
  security_rule {
    ...
    source_port_range          = "*"
    destination_port_range     = "3306"
    source_address_prefix      = var.subnet_db_prefix
    destination_address_prefix = "*"
  }
